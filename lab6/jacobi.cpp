...
Jacobi<T>::Jacobi(const AGHMatrix<T>& A_mat, const AGHMatrix<T>& b_mat, const AGHMatrix<T>& solution, const unsigned& iterations, const unsigned& example_no, const double& eps) : A_mat{A_mat}, b_mat{b_mat}, x_mat{b_mat} {
    ...
    err_data.open("Jacobi_example_" + std::to_string(example_no) + ".txt");
