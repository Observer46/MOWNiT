...
template <typename T> class Jacobi   
{
private:
    AGHMatrix<T> A_mat;
    AGHMatrix<T> b_mat;
...
public:
    ...
    Jacobi(const AGHMatrix<T>& A_mat, const AGHMatrix<T>& b_mat, const AGHMatrix<T>& solution, const unsigned& iterations,const unsigned& example_no, const double& eps = -1.0);
